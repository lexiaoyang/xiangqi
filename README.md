...
战役已经从“一眼看透的普通迷宫”升级为分章节策略迷宫：

- `src/campaign/strategy.ts`：章节、成人向关卡 archetype、战术工具、VIP 档位、体力上限/恢复、奖励倍率等策略规则。
- `src/campaign/retention.ts`：每日挑战、连续挑战、成就、机制图鉴和大师评分等留存循环的纯逻辑。
- `src/campaign/levelSpec.ts`：每关生成章节、复杂度、目标、机制、推荐工具、奖励档和大师标签。
- `src/maze/gameState.ts`：在原有收集、洋流、传送门基础上加入钥匙/锁、陷阱、巡逻、开关、不稳定地块、记忆门、相位门和遗物撤离。
- `src/MazeLevelPlay.tsx`：对局 HUD 展示目标、风险、钥匙、遗物、相位/冻结/脉冲状态和战术工具，结算页展示大师评分、贡献项与改进建议。
- `src/CampaignShell.tsx`：中高复杂度关卡进入前显示战术简报，说明目标、机制、推荐工具和体力成本。

新关卡原则：难度来自规划顺序、资源取舍和风险控制，而不是隐藏规则或不可解随机。`src/campaign/solvability.test.ts` 会验证 1-1000 关在新机制下仍有无付费通关路径。

## 战术商店与 VIP

商店从单纯资源包升级为策略经济：

- 资源补给：金币、体力、提示、撤销，解决基础恢复。
- 战术工具：扫描、冻结、架桥、诱饵、万能钥匙、脉冲、回卷等，直接进入战役工具库存。
- 章节备战：针对机关、相位门、不稳定地块和遗物撤离的组合包。
- VIP 权益：VIP 点数驱动等级，提供体力上限、恢复加速、每日战术包、奖励倍率、扫描半径和额外战术槽。

VIP 不作为通关门槛；核心关卡必须可无 VIP 完成。VIP 的价值是降低重复失败成本、提供更多战术选择，并让长期玩家有清晰成长目标。

## 留存与爽感循环

大厅现在有三条明确目标线：

- 每日挑战：每天确定一个已解锁范围内的策略关卡，展示目标和奖励，完成后发放金币、体力和扫描工具。
- 连续挑战：每日首胜推进 streak，奖励随连续天数提升，鼓励玩家每天回来打一局。
- 长期成长：大师总分、成就目标和机制图鉴会记录玩家是否打得干净、是否掌握新机制、是否完成高价值目标。

通关结算不再只显示星级，而是给出策略大师评分。评分会参考星级、路线效率、风险触发、工具消耗、遗物完成和 VIP 奖励加成，并给出下一步可优化方向。
